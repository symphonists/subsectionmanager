			...
			var edit = function(item, create) {
				...
				if(!item.next('li:not(.template)').hasClass('drawer')) {
					...
					iframe.load(function(event) {
						...
						var height = contents.find('form').outerHeight() || iframe.height();
						...
						// If we are previewing an image inline (Safari), we need to set it's
						// width so that it will appear at a nominal size inside the iframe.
						// Firefox will autosize the inline image to the iframe however, so
						// we need to check if the image has been resized by the browser or not.
						// If it's not, we'll set it to be the width of the iframe.
						if(contents.find('body > img').width() > iframe.width()) {
							contents.find('body > img').css({
								'width': iframe.width()
							});
						}
